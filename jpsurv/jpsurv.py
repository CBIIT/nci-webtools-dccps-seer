...
resultsPath = jpsurvConfig.getAsString('jpsurv.resultsPath')

if not os.path.exists(resultsPath):
    os.makedirs(resultsPath)

UPLOAD_DIR = os.path.join(os.getcwd(), resultsPath)
...
def myExport():
    ...
    def gatherFileNames():
        ...
        (type, dictionary, form, tokenForInput,
         tokenId, txtFile) = extractParameters()
...
def stage3_recalculate():
    ...
    if (switch == True):
        with open(resultsPath + '/cohort_models-'+jpsurvData["tokenId"]+'.json') as data_file:
    ...
    if(os.path.isfile(fname) == False or recalc == "true"):
        ...
        try:
            r.getAllData(UPLOAD_DIR, jpsurvDataString, switch, use_default,
                         resultsPath + '/cohortCombo-'+jpsurvData["tokenId"]+'.json')
            print("GOT RESULTS!")
            status = 200
            out_json = json.dumps({'status': 'OK'})
        except Exception as e:
